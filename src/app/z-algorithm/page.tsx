...
import ZVisualization from '@/components/z-algorithm/ZVisualization';
import ZVariablePanel from '@/components/z-algorithm/ZVariablePanel';
import ZStepControls from '@/components/z-algorithm/ZStepControls';
import { useTheme } from '@/components/ThemeProvider';

export default function ZAlgorithmPage() {
  const [inputPattern, setInputPattern] = useState('abc');
  const [inputText, setInputText] = useState('abcxabcabc');
  const [pattern, setPattern] = useState('abc');
  const [text, setText] = useState('abcxabcabc');
  ...
  const step: ZAlgorithmStep | null = steps[stepIndex] ?? null;
  const prevStep: ZAlgorithmStep | null =
    stepIndex > 0 ? steps[stepIndex - 1] : null;
  ...
  const nextPosition = useCallback(() => {
    ...
    const idx = steps.findIndex(
      (s, i) => i > stepIndex && s.position > currentPos,
    );
  ...
  const prevPosition = useCallback(() => {
    ...
    const idx = steps.findIndex((s) => s.position === targetPos);
    goTo(idx >= 0 ? idx : 0);
  }, [step, steps, goTo]);
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      else if (e.key === 'ArrowUp' || e.key === 'k') prevPosition();
      else if (e.key === 'ArrowDown' || e.key === 'j') nextPosition();
      else if (e.key === ' ') {
        e.preventDefault();
        setIsPlaying((p) => !p);
      }
  ...
  }, [nextStep, prevStepFn, nextPosition, prevPosition]);
      ...
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center gap-6">
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          <div className="flex gap-1">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500/80 animate-bounce [animation-delay:0ms]" />
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500/80 animate-bounce [animation-delay:150ms]" />
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500/80 animate-bounce [animation-delay:300ms]" />
          </div>
        </div>
      </div>
    );
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="flex-1 min-h-75 relative">
            <ZVisualization step={step} prevStep={prevStep} />
          </div>
            ...
            <div>
              ...
              <div className="space-y-1.5 text-xs">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-slate-400">Current Position (k)</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="w-3 h-3 rounded bg-indigo-900/40 ring-1 ring-indigo-500/50" />
                  <span className="text-slate-400">Z-box [l, r]</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="w-3 h-3 rounded bg-blue-900/50 ring-1 ring-blue-500/40" />
                  <span className="text-slate-400">Comparison Match</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="w-3 h-3 rounded bg-cyan-900/40 ring-1 ring-cyan-500/50" />
                  <span className="text-slate-400">Mirror Position (k1)</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="w-3 h-3 rounded bg-emerald-900/40 ring-1 ring-emerald-500/50" />
                  <span className="text-slate-400">Pattern Match Found</span>
                </div>
              </div>
            </div>
          </div>
